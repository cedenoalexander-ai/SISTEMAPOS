...
export default function App() {
  ...
  const [modalHabladorOpen, setModalHabladorOpen] = useState(false);
  const [productoParaHablador, setProductoParaHablador] =
    useState<Product | null>(null);
  const [habladorConfigImpresion, setHabladorConfigImpresion] =
    useState<HabladorConfig | null>(null);
  ...
  // Abrir Modal e Impresión de Hablador de Precios
  const handleAbrirHablador = (producto: Product) => {
    setProductoParaHablador(producto);
    setModalHabladorOpen(true);
  };

  const ejecutarImpresionHablador = (config: HabladorConfig) => {
    setHabladorConfigImpresion(config);
    setTimeout(() => {
      const container = document.getElementById('habladorPrecioContainer');
      if (container) {
        container.classList.remove('hidden');
        window.print();
        container.classList.add('hidden');
      ...
      case 'productos':
        return (
          <ProductsView
            ...
            onAbrirHablador={handleAbrirHablador}
          />
        );
      case 'usuarios':
        return (
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col font-sans">
      ...
      {/* 7. Modal y Contenedor de Impresión de Hablador de Precios */}
      <ModalHabladorPrecio
        isOpen={modalHabladorOpen}
        onClose={() => {
          setModalHabladorOpen(false);
          setProductoParaHablador(null);
        }}
        producto={productoParaHablador}
        tasaBCV={tasaBCV}
        onImprimir={ejecutarImpresionHablador}
      />
      <HabladorPrecioContainer config={habladorConfigImpresion} />
    </div>
  );
}
