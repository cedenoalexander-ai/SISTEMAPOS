...
// =========================================================================
// 11. MODAL Y PLANTILLA DE IMPRESIÓN: HABLADOR DE PRECIOS PARA ESTANTE
// =========================================================================
export interface HabladorConfig {
  producto: Product;
  tasaBCV: number;
  formato: 'termico' | 'gondola' | 'etiqueta';
  copias: number;
...
interface ModalHabladorPrecioProps {
  isOpen: boolean;
  onClose: () => void;
  producto: Product | null;
  tasaBCV: number;
  onImprimir: (config: HabladorConfig) => void;
}

export const ModalHabladorPrecio: React.FC<ModalHabladorPrecioProps> = ({
  isOpen,
  onClose,
  producto,
  tasaBCV,
  ...
  const [formato, setFormato] = useState<'termico' | 'gondola' | 'etiqueta'>('termico');
  const [copias, setCopias] = useState<number>(1);
  const [mostrarFecha, setMostrarFecha] = useState<boolean>(true);
  const [mostrarCodigo, setMostrarCodigo] = useState<boolean>(true);
  const [mostrarTasa, setMostrarTasa] = useState<boolean>(true);

  if (!isOpen || !producto) return null;

  const precioUSD = Number(producto.PrecioVtaRealDivisas || producto.Precio) || 0;
  const precioBs =
    producto.incluir_Costo_en_Bs_o_Divisas === 'Precio Fijo'
      ? Number(producto.PrecioBs) || 0
      : Number(producto.PrecioBs) || precioUSD * tasaBCV;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-3 sm:p-4 backdrop-blur-xs">
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col max-h-[92vh] animate-in fade-in zoom-in-95 duration-150">
        {/* Cabecera */}
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white p-4 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-sm sm:text-base">Imprimir Hablador de Precios</h3>
              <p className="text-[11px] text-blue-100">Etiqueta de estante y anaquel con precios en Bs y Ref USD</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 sm:p-5 overflow-y-auto flex flex-col gap-4">
          {/* Vista Previa del Hablador */}
          <div>
            <span className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-2">
              Vista Previa de Impresión:
            </span>

            {/* Contenedor Visual de Vista Previa */}
            <div className="bg-gray-50 p-4 rounded-xl border-2 border-dashed border-gray-300 flex justify-center items-center">
              <div className="bg-white text-black p-4 rounded-lg shadow-sm border border-gray-400 w-full max-w-[280px] font-sans flex flex-col gap-1 text-center">
                <div className="flex justify-between items-center text-[10px] text-gray-600 border-b border-gray-300 pb-1 mb-1 font-bold">
                  <span>iMtec POS</span>
                  {mostrarFecha && <span>{new Date().toLocaleDateString('es-VE')}</span>}
                </div>
                ...
                {/* Nombre del Producto */}
                <div className="font-black text-xs sm:text-sm text-gray-900 leading-tight uppercase my-1">
                  {producto.Nombre}
                </div>

                {/* PRECIO EN BOLÍVARES EN GRANDE Y NEGRITA */}
                <div className="my-1 py-1.5 px-2 bg-gray-100 rounded-md border border-gray-300">
                  <div className="text-[10px] font-bold text-gray-600 uppercase tracking-wider">
                    PRECIO
                  </div>
                  <div className="text-2xl sm:text-3xl font-black text-gray-950 tracking-tight leading-none">
                    {formatearBS(precioBs)}
                  </div>
                </div>

                {/* REFERENCIA EN DÓLARES ABAJO */}
                <div className="flex justify-center items-center gap-1.5 text-xs sm:text-sm font-black text-gray-800 border-t border-gray-200 pt-1.5">
                  <span className="text-[10px] font-bold uppercase text-gray-600">REF:</span>
                  <span className="text-base font-black text-blue-700 bg-blue-50 px-2 py-0.5 rounded border border-blue-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200 text-xs">
            <div>
              <label className="block font-bold text-gray-700 mb-1">Formato de Impresión:</label>
              <select
                value={formato}
                onChange={(e) => setFormato(e.target.value as any)}
                className="w-full p-2 border border-gray-300 rounded-lg bg-white font-medium outline-none focus:ring-2 focus:ring-blue-500"
              >
                <option value="termico">Impresora Térmica (80mm / 58mm)</option>
                <option value="gondola">Etiqueta Estante / Góndola (Horizontal)</option>
                <option value="etiqueta">Mini Etiqueta Adhesiva (5x3 cm)</option>
              </select>
            </div>

            <div>
              <label className="block font-bold text-gray-700 mb-1">Número de Copias:</label>
              <div className="flex items-center gap-2">
                <input
                  type="number"
                  min={1}
                  ...
                  className="w-full p-2 border border-gray-300 rounded-lg bg-white font-bold text-center outline-none"
                />
                <span className="text-gray-500 font-medium whitespace-nowrap">etiqueta(s)</span>
              </div>
            </div>

            <div className="sm:col-span-2 flex flex-wrap gap-4 pt-1 border-t border-gray-200">
        ...
        {/* Acciones */}
        <div className="bg-gray-100 p-3 sm:p-4 border-t border-gray-200 flex justify-end gap-2">
          <button
            onClick={onClose}
            className="px-4 py-2.5 bg-gray-200 hover:bg-gray-300 font-bold text-xs rounded-xl text-gray-700 transition"
          >
            Cancelar
          </button>
          <button
            onClick={handlePrint}
            className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl shadow-md flex items-center gap-2 cursor-pointer transition active:scale-98"
          >
...
// =========================================================================
// 12. COMPONENTE DE IMPRESIÓN OCULTO PARA HABLADOR DE PRECIOS
// =========================================================================
interface HabladorPrecioContainerProps {
  config: HabladorConfig | null;
}

export const HabladorPrecioContainer: React.FC<HabladorPrecioContainerProps> = ({ config }) => {
  if (!config || !config.producto) return null;

  const { producto, tasaBCV, formato, copias, mostrarFecha, mostrarCodigo, mostrarTasa } = config;
  const precioUSD = Number(producto.PrecioVtaRealDivisas || producto.Precio) || 0;
  const precioBs =
    producto.incluir_Costo_en_Bs_o_Divisas === 'Precio Fijo'
      ? Number(producto.PrecioBs) || 0
  ...
  const copiasArray = Array.from({ length: Math.max(1, copias) });

  return (
    <div id="habladorPrecioContainer" className="hidden">
      {copiasArray.map((_, idx) => (
        <div
          key={idx}
          style={{
            pageBreakInside: 'avoid',
            pageBreakAfter: idx < copiasArray.length - 1 ? 'always' : 'auto',
            width: formato === 'termico' ? '72mm' : formato === 'gondola' ? '90mm' : '55mm',
            padding: '4mm',
        ...
        >
          ...
          {/* Código del Producto */}
          {mostrarCodigo && (
            <div style={{ fontSize: '10px', fontFamily: 'monospace', margin: '2px 0' }}>
              CÓD: {producto.Codigo}
            </div>
          )}
          ...
          >
            <span style={{ fontSize: '12px', fontWeight: 'bold' }}>REF:</span>
