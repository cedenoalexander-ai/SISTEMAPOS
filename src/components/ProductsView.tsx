...
interface ProductsViewProps {
  ...
  onAbrirHablador?: (producto: Product) => void;
}
              ...
              <button
                ...
                onClick={() =>
                  onAbrirHablador({
                    ...
                    unidadesxEmpaque: parseFloat(unidadesEmpaque) || 1,
                    Impuesto: parseFloat(impuesto) || 0,
                    Porcentaje_a_Ganar: parseFloat(porcentajeGanar) || 0,
                    PrecioVtaRealDivisas: parseFloat(precioRealDivisas) || 0,
                    Precio: parseFloat(precioRealDivisas) || 0,
                    PrecioBs: parseFloat(precioBs) || 0,
                    Stock: parseFloat(stock) || 0,
                  })
                }
                className="bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 font-bold px-3 py-3 rounded-xl transition text-xs flex items-center gap-1.5 cursor-pointer shadow-xs"
                title="Imprimir Hablador de Precios"
              >
                <Tag className="w-4 h-4 text-indigo-600" />
                <span className="hidden sm:inline">Hablador</span>
              </button>
            )}
                ...
                >
                  ...
                  <div className="flex gap-1 items-center">
                    {onAbrirHablador && (
                      <button
                        type="button"
                        onClick={() => onAbrirHablador(p)}
                        className="bg-indigo-600 hover:bg-indigo-700 text-white p-1.5 rounded-lg cursor-pointer transition shadow-xs"
                        title="Imprimir Hablador de Precios (Bs / Ref USD)"
                      >
